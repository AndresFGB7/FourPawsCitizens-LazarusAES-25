...
public class PetService {
    ...
    public Optional<PetPOJO> findPet(Integer id) {
        ...
        if (pet.isPresent()) {
           Integer id2 = Math.toIntExact(pet.get().getOwner_id().getPersonId());
            ...
            return Optional.of(new PetPOJO(pet.get().getPet_id(), pet.get().getMicroship(), pet.get().getName(), pet.get().getSpecies(), pet.get().getRace(), pet.get().getSize(), pet.get().getSex(), pet.get().getPicture(), id2));
        }
    ...
    public List<PetPOJO> ListPets(){
        ...
        for(Pet pet : pets){
            petPOJOS.add(new PetPOJO(pet.getPet_id(),pet.getMicroship(),pet.getName(),pet.getSpecies(),pet.getRace(),pet.getSize(),pet.getSex(),pet.getPicture(),Math.toIntExact(pet.getOwner_id().getPersonId())));
        }
